export interface PolicyFields {
  ...
  allowed_hours_utc: string | null; // int4range e.g. "[9,18)"
...
function isInHoursRange(hour: number, from: number, to: number): boolean {
  ...
  // Wrapping range, e.g. [22,6) means 22-23 and 0-5
  return hour >= from || hour < to;
}

/**
 * Evaluate a single payment against a policy.
 * Short-circuits on first match per the defined order of checks.
 */
export function evaluate(
  ...
  // 4. Allowlist address (if set, recipient must be in it)
  ...
  // 5. Allowlist username (if set, resolved username must be in it)
