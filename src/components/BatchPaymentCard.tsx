...
import { ExternalLink, ChevronRight, CheckCircle2, XCircle } from 'lucide-react';
import { Batch, BatchPayment, useBatchPayments } from '@/hooks/useBatchPayments';
        ...
        <div className="glass-card p-5 rounded-2xl flex flex-col gap-4 border border-border/50 hover:border-primary/30 transition-colors">
            ...
            <div className="flex justify-between items-start gap-2">
                <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{new Date(batch.created_at).toLocaleString()}</p>
                </div>
            ...
            <div className="space-y-1.5">
                <div className="flex justify-between text-xs text-muted-foreground">
                    ...
                    <span className="font-semibold text-foreground">{batch.total_amount.toFixed(2)} USDC</span>
                </div>
                    ...
                    return (
                        <div key={i} className="flex items-center justify-between text-sm">
                            <div className="flex items-center gap-2 min-w-0">
                                ...
                                {rec.label && <span className="text-xs text-muted-foreground truncate">· {rec.label}</span>}
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                                <span className="font-medium text-xs">{rec.amount} USDC</span>
                                {payment?.tx_hash && (
                                    <a
            ...
            <div className="pt-3 border-t border-border">
                <a
                    ...
                    className="flex items-center justify-center gap-2 w-full rounded-xl bg-secondary hover:bg-muted py-2.5 text-sm font-medium transition-colors"
                >
                    View Receipt <ChevronRight size={15} />
                </a>
            </div>
        </div>
    );
